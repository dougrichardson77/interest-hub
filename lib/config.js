...
export const DEFAULT_REFRESH_RATE_LIMIT_WINDOW_MS = 60_000;
export const DEFAULT_REFRESH_RATE_LIMIT_MAX = 20;
export const DEFAULT_JSON_BODY_LIMIT_BYTES = 1024 * 64;
...
export function getRuntimeConfig(env = process.env) {
  ...
  const corsOriginAllowlist = splitCsv(env.CORS_ORIGIN_ALLOWLIST);
  const localDataDir = env.LOCAL_DATA_DIR || "";
  ...
  return {
    ...
    corsOriginAllowlist,
    rateLimitWindowMs: toPositiveInt(env.RATE_LIMIT_WINDOW_MS, DEFAULT_API_RATE_LIMIT_WINDOW_MS),
    rateLimitMaxRequests: toPositiveInt(env.RATE_LIMIT_MAX_REQUESTS, DEFAULT_API_RATE_LIMIT_MAX),
    refreshRateLimitWindowMs: toPositiveInt(
      env.REFRESH_RATE_LIMIT_WINDOW_MS,
      DEFAULT_REFRESH_RATE_LIMIT_WINDOW_MS
    ),
    refreshRateLimitMaxRequests: toPositiveInt(
      env.REFRESH_RATE_LIMIT_MAX_REQUESTS,
      DEFAULT_REFRESH_RATE_LIMIT_MAX
    ),
    jsonBodyLimitBytes: toPositiveInt(env.JSON_BODY_LIMIT_BYTES, DEFAULT_JSON_BODY_LIMIT_BYTES),
    appVersion: cleanText(env.APP_VERSION || "0.1.0"),
    minClientVersion: cleanText(env.MIN_CLIENT_VERSION || "1.0.0"),
    localDataDir
  };
}
...
function normalizeAppEnv(value) {
  const normalized = String(value || DEFAULT_APP_ENV).trim().toLowerCase();
